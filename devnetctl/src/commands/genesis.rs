...
pub fn execute(command: GenesisCommands) -> Result<String, Error> {
    match command {
        GenesisCommands::Random {
            initial_seed,
            num_validator,
            num_genesis_validator,
            chain_id,
        } => {
            generate_random_genesis(
                initial_seed,
                num_validator,
                num_genesis_validator,
                chain_id,
        ...
        GenesisCommands::FromPub { initial_seed, pkfile } => {
            generate_genesis_from_pubkey_file(initial_seed, pkfile)
        }
    }
}

fn generate_random_genesis(
    initial_seed: String,
    num_validator: usize,
    num_genesis_validator: usize,
    chain_id: u32,
) -> Result<String, Error> {
    let initial_seed: H256 = initial_seed.parse()
        .expect("invalid initial seed");
    
    if num_genesis_validator > num_validator {
        panic!("The number of genesis validators cannot be more than the total number of validators: {} > {}", num_genesis_validator, num_validator);
    }
    ...
    for i in 0..num_validator {
        ...
        let vrf_public_key_str = vrf_public_key.to_encoded_string().unwrap();
    ...
    save_initial_nodes_to_file(
        ...
        GenesisPosState {
            ...
            initial_committee: initial_committee
                .iter()
                .map(|(node_id, voting_power)| (node_id.addr, *voting_power))
                .collect(),
            initial_seed,
        },
    );
    generate_genesis_from_public_keys(initial_committee);
    Ok("PoS genesis configuration generated successfully".into())
}

fn generate_genesis_from_pubkey_file(
    initial_seed: String,
    pkfile: PathBuf,
) -> Result<String, Error> {
    let initial_seed: H256 = initial_seed.parse()
    ...
    let mut public_key_file = File::open(&pkfile)?;
    ...
    while let Some(key_str) = lines.next() {
        ...
        let public_key = ConsensusPublicKey::from_encoded_string(key_array[0])
            .unwrap();
        let vrf_public_key = ConsensusVRFPublicKey::from_encoded_string(key_array[1])
            .unwrap();
    ...
    generate_genesis_from_public_keys(initial_committee);
    Ok("PoS genesis configuration generated from public keys successfully".into())
}
...
fn generate_genesis_from_public_keys(public_keys: Vec<(NodeID, u64)>) {
    ...
    let write_set_payload = WriteSetPayload::Direct(change_set);
    let genesis_transaction = Transaction::GenesisTransaction(write_set_payload);
        ...
        for nonce in 0..node.voting_power {
            electing_heap.push((
                HashValue::sha3_256_of(
                    &[&buffer as &[u8], &(nonce as u64).to_be_bytes()].concat(),
                ),
                node_id.addr,
            ));
        }
